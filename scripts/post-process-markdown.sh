...
find ../ -type f -name "*.md" | while read -r file; do
  ...
  sed -i '' 's/URL://g' "$file"
    sed -i '' 's/```text/```/g' "$file"
  sed -i '' 's/용어 해설 (Glossary)/용어 해설/g' "$file"
  sed -i '' 's/CTA (Call to Action)/CTA/g' "$file"
  ...
  sed -i '' -e 's/^### ## /### /g' \
    ...
    -e 's/# 제목:/#/g' \
    -e 's/URL://g' \
    -e 's/용어 해설 (Glossary)/용어 해설/g' \
    -e 's/CTA (Call to Action)/CTA/g' \
    -e 's/핵심 요약 (Executive Summary)/핵심 요약/g' \
    -e 's/핵심 요약 (Hook)/핵심 요약/g' \
    -e 's/상세 내용 (Detailed Content)/상세 내용/g' \
    -e 's/상세 내용 (Storyline)/상세 내용/g' \
    -e 's/참고 자료 (References & Further Reading)/참고 자료/g' \
    -e 's/개요 (Overview)/개요/g' \
    -e 's/YouTube 영상.*자료: //g' \
    -e 's/다음은 YouTube 영상의 자막 내용을.*학습 자료입니다.//g' \
    -e 's/다음은 YouTube 영상의 자막 내용을.*자료로 재구성한 문서입니다.//g' \
    -e 's/```text/```/g' \
    -e '/^$/N;/^\n$/N;/^\n\n$/d' "$file"
done
echo "Done."